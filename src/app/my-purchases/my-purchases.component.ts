...
import { IGX_LIST_DIRECTIVES, IgxButtonDirective, IgxIconComponent, IgxRippleDirective } from 'igniteui-angular';
...
@Component({
  ...
  imports: [IGX_LIST_DIRECTIVES, IgxButtonDirective, IgxRippleDirective, IgxIconComponent],
...
export class MyPurchasesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.movieAppService.getMyPurchasesList().pipe(takeUntil(this.destroy$)).subscribe(
      data => this.movieAppMyPurchases = data
    );
  }
